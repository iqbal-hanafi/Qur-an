jadwal_shalat = function(kota){
		...
		$.ajax(
			`http://api.aladhan.com/v1/timingsByCity?city=${kota}&country=Indonesia&method=8`, {method: `GET`, async:false, headers: {'User-Agent':'Mozilla/5.0','Connection':'keep-alive','Content-Type':'application/json'}).done((data) => {
			d = data.data.timings;
		}).fail(() => {
			d = {'error': true};
		});
...
$(document).ready(() => {
	...
	$(document).on(`click`, 'div[data-index]', function(){
		...
		$.ajax(
			`https://al-quran-8d642.firebaseio.com/surat/${i}.json`
			,{method: `GET`})
		.done((data) => {
